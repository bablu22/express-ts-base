...
export class OtpService {
  private static readonly OTP_PREFIX = 'otp:verify:';
  private static readonly OTP_TTL_SECONDS = 600; // 10 minutes

  constructor(private readonly redis: Redis = RedisClient.getInstance()) {}

  /**
   * Generates a 6-digit OTP code and stores it in Redis with key `otp:verify:<email>`.
   */
  async generateOtp(email: string): Promise<string> {
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const key = `${OtpService.OTP_PREFIX}${email.toLowerCase().trim()}`;
  ...
  /**
   * Verifies the OTP code for the given email from Redis.
